...
while (i <= n) && (k <= m)
    ...
    %这是matlab提供的库函数gfrank，我把它拿到这儿加上my程mygfrank，不过gfrank函数用的是a(j, vec) = mod(a(j, vec) + a(i, vec) * (p - a(j, k)),
% p)这种莫名其妙的运算，当素数域很小时，还可以，当大了之后，rank总是为0，我们自己实现了素数域上的除法运算，这样就可以按照王磊的方法将素数域里的rank求出来。
%所以在素数时，我们用worank函数提到gfrank函数
